...
export default function Tree({
  ...
  onReorder?: (sourcePath: number[], targetPath: number[], position: "before" | "after" | "inside") => void
  ...
  if (item.type === "file") {
    return (
      ...
      >
        {dragOver === "before" && <div className="absolute top-0 left-0 right-0 h-0.5 bg-blue-500 z-10" />}
        {dragOver === "after" && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-500 z-10" />}
        ...
        >
          <SidebarMenuItem className="cursor-pointer rounded" onClick={() => onSelect?.(item.slug, item.id, item.type)}>
            <SidebarMenuButton
              className={`text-xs rounded ${
                selectedId === item.id ? "bg-blue-300/30 hover:bg-blue-300/30" : "bg-transparent hover:bg-cyan-300/30"
  ...
  return (
    ...
    >
      ...
      {dragOver === "after" && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-500 z-10" />}
      {dragOver === "inside" && (
        <div className="absolute inset-0 bg-blue-500/20 border-2 border-blue-500 rounded z-0 pointer-events-none" />
      )}
